...
This pattern is an evolution of the [canonical LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) by Andrej Karpathy that replaces the single flat namespace with physically separated per-domain directories. A flat layout works at ~30-60 pages; beyond that, per-session reading cost scales with total page count. Domain atomization fixes this by making per-session reads scale with the **touched** domain only, and replaces honor-system scoping with mechanical enforcement. Extension patterns on the roadmap — memory lifecycle, the broader event-hook taxonomy, ingest-time privacy filters — are informed by [rohitg00's LLM Wiki v2](https://gist.github.com/rohitg00/2067ab416f7bbe447c1977edaaa681e2), a production-lessons sequel to the canonical pattern distilled from building the [`agentmemory`](https://github.com/rohitg00/agentmemory) engine.
